...
# 1. Add CSS file link if not present
if 'custom-3d.css' not in html:
    html = html.replace('</head>', '    <link rel="stylesheet" href="css/components/custom-3d.css">\n</head>')
...
tech_html += '\n                </div>'
# The regex looks for <div class="techlaw__grid"> followed by anything until </div> \s* </div> \s* </section>
# Actually, let's use a very safe replace
...
# fallback if why__grid didn't have style inline before:
if '<div class="why__grid">' in html:
     html = re.sub(r'<div class="why__grid">[\s\S]*?(?=</section>)', why_html + '\n            ', html)
...
print("Updated index.html correctly this time!")
